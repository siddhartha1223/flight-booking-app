...
required_env_vars = ['FLASK_SECRET_KEY', 'SENDGRID_API_KEY', 'FROM_EMAIL']
...
train_options = {
    'Bangalore': { '1': ('BNC-MAS Express', '12609', 'Chennai'), '2': ('BNC-HYB Superfast', '12785', 'Hyderabad'), '3': ('BNC-TPTY Express', '16203', 'Tirupati'), '4': ('BNC-MYS Express', '16517', 'Mysore'), '5': ('BNC-CBE Intercity', '12677', 'Coimbatore') },
    'Chennai': { '1': ('MAS-BNC Mail', '12657', 'Bangalore'), '2': ('MAS-CSTM Express', '11042', 'Mumbai'), '3': ('MAS-HWH Coromandel', '12841', 'Kolkata'), '4': ('MAS-SBC Shatabdi', '12027', 'Bangalore'), '5': ('MAS-VSKP Express', '22801', 'Visakhapatnam') },
...
def verify_recaptcha(response):
    if not RECAPTCHA_SECRET_KEY: return True, "Success (reCAPTCHA is disabled)"
...
def init_db():
    try:
        ...
        TABLES['customers'] = ('''CREATE TABLE IF NOT EXISTS customers (
            ...
            FOREIGN KEY (user_id) REFERENCES users(id) ON DELETE SET NULL, INDEX idx_user_id (user_id), INDEX idx_status (status)
        )''')
        ...
        TABLES['seats'] = ('''CREATE TABLE IF NOT EXISTS seats (
            id INT AUTO_INCREMENT PRIMARY KEY, type VARCHAR(10) NOT NULL, number VARCHAR(20) NOT NULL, status VARCHAR(20) NOT NULL,
...
def signup():
    if request.method == 'POST':
        ...
        if not is_valid: flash(msg, 'error'); return redirect(url_for('signup'))
        if not (validate_name(name) and validate_phone(phone) and validate_email(email) and re.match(r'^(?=.*[A-Za-z])(?=.*\d)[A-Za-z\d]{6,}$', password)):
            flash('Invalid data provided. Please check all fields.', 'error'); return redirect(url_for('signup'))
...
def book_flight():
    if request.method == 'POST':
        ...
        try:
            ...
            if is_unavailable:
                flash('Selected seat is not available.', 'error'); db.rollback(); cursor.close(); return redirect(url_for('book_flight'))
            
            ticket_id = f'FL{name[:3].upper()}{random.randint(100, 999)}'
            fare = random.randint(5000, 7000) if flight_map[destination_key] in ('Bangalore', 'Chennai', 'Mumbai', 'Goa', 'Pune') else random.randint(30000, 50000)
...
def book_train():
    if request.method == 'POST':
        ...
        if not all([validate_name(name), validate_phone(phone), validate_email(email), train_choice, compartment, seat, travel_date]):
            flash('Invalid input provided. Please check all fields.', 'error'); return redirect(url_for('book_train'))
        food_quantities = {item: int(request.form.get(f'quantity_{item}', '1')) for item in food_items}
        ...
        try:
            ...
            is_unavailable = (not seat_data or seat_data['status'] == 'booked' or (seat_data['status'] == 'locked' and seat_data['locked_until'] and seat_data['locked_until'] > datetime.now()))
            if is_unavailable:
                flash('Selected seat is not available.', 'error'); db.rollback(); cursor.close(); return redirect(url_for('book_train'))

            train_name, train_number, destination = None, None, None
            for trains in train_options.values():
                if train_choice in trains:
                    train_name, train_number, destination = trains[train_choice]; break
            if not train_name:
                flash('Invalid train selected.', 'error'); db.rollback(); cursor.close(); return redirect(url_for('book_train'))
            ...
            session['booking'] = {'type': 'train', 'ticket_id': ticket_id, 'user_id': current_user.id if current_user.is_authenticated else None,
                                  'name': name, 'phone': phone, 'email': email, 'train_name': train_name, 'train_number': train_number, 'destination': destination,
                                  'compartment': compartment, 'seat': seat, 'fare': fare, 'food_cost': food_cost, 'food_orders': food_orders, 'travel_date': travel_date}
            ...
            flash('Database error occurred.', 'error'); logger.error(f"DB error in book_train POST: {err}"); db.rollback()
    ...
    try:
        ...
        flash('Error fetching train data.', 'error'); logger.error(f"DB error in book_train GET: {err}")
        return redirect(url_for('home'))

@app.route('/book_combined', methods=['GET', 'POST'])
def book_combined():
    if request.method == 'POST':
        # NOTE: This is a complex function merging the logic from book_flight and book_train.
        # The logic involves validating and locking two seats in a single transaction.
        # For simplicity in this final version, it redirects. A full implementation would be needed for this feature.
        flash('Combined booking POST logic is not fully implemented in this version.', 'info')
        return redirect(url_for('home'))
    ...
    try:
        ...
        flash('Error fetching data for combined booking.', 'error'); logger.error(f"DB error in book_combined GET: {err}")
        return redirect(url_for('home'))
...
def get_train_options():
    try:
        ...
        if flight_destination_key in flight_map:
...
def payment_success():
    ...
    try:
        ...
        for item, qty, cost in booking.get('food_orders', []):
            cursor.execute('INSERT INTO food_orders (ticketID, food_item, quantity, food_cost) VALUES (%s, %s, %s, %s)', (ticket_id, item, qty, cost))

        otp = random.randint(100000, 999999)
        cursor.execute('INSERT INTO verification_tokens (ticket_id, otp, created_at) VALUES (%s, %s, %s)', (ticket_id, otp, datetime.now()))
...
def verify_booking(ticket_id):
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            elif ticket_id.startswith('CB'): table_to_update = 'combined_booking'
...
def download_ticket(ticket_id):
    if not validate_ticket_id(ticket_id): flash('Invalid ticket ID format.', 'error'); return redirect(url_for('home'))
    try:
        db = get_db()
        cursor = db.cursor(dictionary=True)
        booking, booking_type = None, None
        if ticket_id.startswith('FL'):
            cursor.execute('SELECT * FROM customers WHERE ticketID = %s AND status = %s', (ticket_id, 'confirmed')); booking = cursor.fetchone(); booking_type = 'flight' if booking else None
        elif ticket_id.startswith('TR'):
            cursor.execute('SELECT * FROM train_bookings WHERE ticketID = %s AND status = %s', (ticket_id, 'confirmed')); booking = cursor.fetchone(); booking_type = 'train' if booking else None
        elif ticket_id.startswith('CB'):
            cursor.execute('SELECT * FROM combined_booking WHERE ticketID = %s AND status = %s', (ticket_id, 'confirmed')); booking = cursor.fetchone(); booking_type = 'combined' if booking else None

        if not booking:
            flash('Booking not found or not confirmed.', 'error'); cursor.close(); return redirect(url_for('home'))
...
def admin():
    try:
        ...
        cursor.execute('SELECT * FROM train_bookings')
        train_bookings = cursor.fetchall()
        cursor.execute('SELECT * FROM combined_booking')
...
def edit_booking(ticket_id):
    ...
    table_name = None
    if ticket_id.startswith('FL'): table_name = 'customers'
    elif ticket_id.startswith('TR'): table_name = 'train_bookings'
    elif ticket_id.startswith('CB'): table_name = 'combined_booking'
    if not table_name: flash('Invalid ticket prefix.', 'error'); return redirect(url_for('admin'))
...
def cancel_booking(ticket_id):
    ...
    try:
        db.start_transaction()
        booking, table_name, email = None, None, None
        cursor.execute('SELECT gmail, status FROM customers WHERE ticketID = %s', (ticket_id,)); booking = cursor.fetchone(); table_name = 'customers'
        if not booking: cursor.execute('SELECT gmail, status FROM train_bookings WHERE ticketID = %s', (ticket_id,)); booking = cursor.fetchone(); table_name = 'train_bookings'
        if not booking: cursor.execute('SELECT gmail, status FROM combined_booking WHERE ticketID = %s', (ticket_id,)); booking = cursor.fetchone(); table_name = 'combined_booking'
        
        if not booking: flash('Booking not found.', 'error'); db.rollback(); cursor.close(); return redirect(url_for('admin'))
        if booking['status'] == 'cancelled': flash('Booking already cancelled.', 'warning'); db.rollback(); cursor.close(); return redirect(url_for('admin'))

        email = booking['gmail']
        query = f'UPDATE {table_name} SET status = %s WHERE ticketID = %s'
        cursor.execute(query, ('cancelled', ticket_id))
        cursor.execute('UPDATE seats SET status = "available", locked_until = NULL, ticket_id = NULL WHERE ticket_id = %s', (ticket_id,))
...
def check_pnr():
    if request.method == 'POST':
        ...
        try:
            ...
            cursor = db.cursor(dictionary=True)
            booking, booking_type = None, None
            cursor.execute('SELECT * FROM customers WHERE ticketID = %s', (ticket_id,)); booking = cursor.fetchone(); booking_type = 'flight'
            if not booking: cursor.execute('SELECT * FROM train_bookings WHERE ticketID = %s', (ticket_id,)); booking = cursor.fetchone(); booking_type = 'train'
            if not booking: cursor.execute('SELECT * FROM combined_booking WHERE ticketID = %s', (ticket_id,)); booking = cursor.fetchone(); booking_type = 'combined'
            
            if not booking: flash('No booking found.', 'error'); cursor.close(); return redirect(url_for('check_pnr'))
...
def user_cancel_booking():
    if request.method == 'POST':
        ...
        if not validate_ticket_id(ticket_id) or not validate_phone(phone):
            flash('Invalid input.', 'error'); return redirect(url_for('user_cancel_booking'))
        
        db = get_db()
        cursor = db.cursor(dictionary=True)
        try:
            db.start_transaction()
            table_to_update, booking = None, None
            cursor.execute('SELECT * FROM customers WHERE ticketID = %s AND user_id = %s', (ticket_id, current_user.id)); booking = cursor.fetchone(); table_to_update = 'customers'
            if not booking: cursor.execute('SELECT * FROM train_bookings WHERE ticketID = %s AND user_id = %s', (ticket_id, current_user.id)); booking = cursor.fetchone(); table_to_update = 'train_bookings'
            if not booking: cursor.execute('SELECT * FROM combined_booking WHERE ticketID = %s AND user_id = %s', (ticket_id, current_user.id)); booking = cursor.fetchone(); table_to_update = 'combined_booking'

            if not booking: flash('Booking not found or you are not authorized.', 'error'); db.rollback(); cursor.close(); return redirect(url_for('user_cancel_booking'))
            if booking['status'] == 'cancelled': flash('Booking already cancelled.', 'warning'); db.rollback(); cursor.close(); return redirect(url_for('booking_history'))
            if booking['phnum'] != phone: flash('Phone number does not match.', 'error'); db.rollback(); cursor.close(); return redirect(url_for('user_cancel_booking'))
            ...
            message = Mail(from_email=FROM_EMAIL, to_emails=booking['gmail'], subject='Booking Cancellation Confirmation',
                           html_content=f'Your booking ({ticket_id}) has been successfully cancelled.')
            sendgrid_client = SendGridAPIClient(SENDGRID_API_KEY)
            sendgrid_client.send(message)
