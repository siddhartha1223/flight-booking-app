...
    cursor = None
    try:
        conn = get_db_connection() # Using the helper function is cleaner
        cursor = conn.cursor(dictionary=True)
        booking = None
        booking_type = None
        
        # ... (Your logic to fetch the booking details is correct) ...
...
        # ... (and so on for train and combined)
...
        
        # --- NEW: Email the PDF using SendGrid ---
        try:
            encoded_pdf = base64.b64encode(pdf_bytes).decode()

            message = Mail(
                from_email='your-verified-sender@yourdomain.com', # Use your verified email
                to_emails=booking['gmail'],
                subject=f"Your Booking Confirmation and Ticket (ID: {ticket_id})",
                html_content='<strong>Thank you for your booking!</strong><br>Your ticket is attached.'
            )
            
            message.attachment = {
                "content": encoded_pdf,
...
        conn.start_transaction()
        
        # ... (Your existing code to find the booking is correct) ...
        table_to_update = None
        # (This part of your code finds the right table and booking details)
        
        if not booking:
...
        email = booking['gmail']
        
        # Update the booking and seat status
        query = f'UPDATE {table_to_update} SET status = %s WHERE ticketID = %s'
...
        # --- NEW: Send cancellation email via SendGrid ---
        message = Mail(
            from_email='your-verified-sender@yourdomain.com', # Use your verified email
            to_emails=email,
            subject='Your Booking Has Been Cancelled by Admin',
            html_content=f'<strong>Your booking with Ticket ID: {ticket_id} has been cancelled by an administrator.</strong>'
        )
        sendgrid_client = SendGridAPIClient(os.environ.get('SENDGRID_API_KEY'))
        sendgrid_client.send(message)
        # --- End of new code ---
        
        conn.commit()
        flash('Booking cancelled successfully and notification sent.', 'success')
        logger.info(f"Booking cancelled by admin: ticket_id={ticket_id}")

    except Exception as e: # Changed to catch any error, including SendGrid's
...
            conn.start_transaction()
            
            # ... (Your logic to find the correct booking for the current user is correct) ...
...
                from_email='your-verified-sender@yourdomain.com', # Use your verified email
                to_emails=booking['gmail'],
                subject='Your Booking Cancellation Confirmation',
                html_content=f'<strong>Your booking with Ticket ID: {ticket_id} has been successfully cancelled.</strong>'
            )
            sendgrid_client = SendGridAPIClient(os.environ.get('SENDGRID_API_KEY'))
            sendgrid_client.send(message)
            # --- End of new code ---
...
            
        except Exception as e: # Changed to catch any error
            flash('Error cancelling booking.', 'error')
